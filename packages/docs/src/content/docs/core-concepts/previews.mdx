...
With `playback` set, the component renders a thumbnail image during upload, falls back to that thumbnail at `ready` if no `playbackUrl` exists, and transitions to a `<video>` element once a `playbackUrl` is set.
